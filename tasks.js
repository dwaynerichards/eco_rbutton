...
task("checkChest", "Checks chest", async () => {
  ...
  signers[0].RButton.test();
  signers[0].RButton.test();
  let currentVault = await signers[0].RButton.getVaultEth();
  ...
  let laterVault = await signers[0].RButton.getVaultEth();
  laterVault = parseInt(laterVault._hex, 16);
  console.log("later VAult:", laterVault);
  tx = await tx.wait();
  const withdraw = tx.events[0].args.totalEth;
  console.log("withdwaw", withdraw);
